...
- **OAuth tokens:** Stored locally in `data/gmail_token.json` (Gmail) and `data/calendar_token.json` (Calendar). Both CLI and dashboard read from these same files (Gmail unified by CAR-198; Calendar unified by CAR-206). CLI re-auth (`python -m cli auth gmail`, `python -m cli calendar`, or the one-liner in `docs/solutions/best-practices/oauth-reauth.md`) is the single source of truth — no env-var duplication, no manual sync, no dev-server restart needed.
