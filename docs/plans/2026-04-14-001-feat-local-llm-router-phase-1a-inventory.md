...
- **Resolved task IDs:** 21 call-site + `embed_default` = **22 TASK_CONFIG keys**
  - 6 R9 structured-local task IDs
  - 15 R10 Claude-default task IDs (includes `skill_study_plan` — structurally R9 but Claude-only due to `web_search`)
  - 1 embed task ID
...
## R9 Structured-Local Tier (6 task IDs, 6 calls)
...
## R10 Claude-Default Tier (15 task IDs, 15 calls)

| Task ID | File | Call | Model (pre-migration) | max_tokens | temperature | fallback_policy | schema |
|---------|------|------|-----------------------|-----------|-------------|-----------------|--------|
| `skill_study_plan` | `src/intel/skill_analyzer.py` | line 189 | `"claude-sonnet-4-6"` | 4096 | (default) | `allow` | JSON array |
...
**Notes:**
- `skill_study_plan`: structurally R9 (schema-validated extraction) but its system prompt requires `web_search` (Claude-only tool). **TASK_MODEL_MAP must remain `"claude"` — do not flip to `"local"` in Unit 3.** Phase 1b promotion decision requires either a local web-search shim or a reformulated prompt that omits the search requirement.
