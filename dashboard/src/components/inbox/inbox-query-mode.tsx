...
const RECENT_QUERIES_KEY = "careerpilot_inbox_recent_queries"
const MAX_RECENT = 5
...
const SYNTAX_HELP = `Filter Syntax:
  ...
  from:name             from_name or from_email contains "name"
  sender:name           alias for from:
  domain:example.com    from_domain contains "example.com"
  body:keyword          body_preview contains "keyword"
  category:recruiter    category filter (recruiter, interview, offer, etc.)
  linked:yes            only linked emails (also: linked:no)
  date:<7d              received within last 7 days (also: 3d, 14d, 30d)
  autotrack:tracked     auto-tracked emails (also: autotrack:prompted)
  -keyword              exclude emails containing "keyword"
  "exact phrase"        match exact phrase in subject, sender, or body
  keyword               bare word matches subject, sender, or body

Category shortcuts:
  recruiter → recruiter_outreach    interview → interview_request
  followup → follow_up              alert → job_alert
  rejected → rejection

Examples:
  subject:interview from:linkedin linked:yes
  domain:lilly.com category:recruiter date:<7d
  "systems engineer" -newsletter autotrack:tracked`

export function InboxQueryMode({
  queryString,
  onQueryChange,
  onToggle,
  totalCount,
    ...
    <div className="space-y-3">
      ...
      <div className="p-4 rounded-xl bg-zinc-50/50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-700 space-y-3">
        ...
        <input
          ...
          placeholder='subject:interview from:linkedin domain:lilly.com linked:yes date:<7d'
...
export function InboxQueryModeToggle({ onClick }: { onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
