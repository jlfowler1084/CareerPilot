...
export default function OverviewContent({
  initialApplications,
  initialEvents,
  initialNewMatchCount,
}: OverviewContentProps) {
  const { applications: hookApplications, loading } = useApplications()
  // Use server data while hook is loading, then switch to hook data for real-time
  const applications = loading && initialApplications ? initialApplications : hookApplications
  const isLoading = loading && !initialApplications
  ...
  const [newMatchCount, setNewMatchCount] = useState(initialNewMatchCount)
  useEffect(() => {
